Whenever I view the calendar and then switch to the camera, I get
a black screen. This issue only arises whenever I view the calendar.
If I reload the app and do not click on the calendar, the camera works
perfectly fine
-Eliseo

--Fixed--
Figure out how to fix image quality issues after running opencv script on image
-Eliseo
--Fixed--


--Fixed
After taking a picture, we should change the view to the calendar view so that the user can see the updated calendar
--Fixed--

